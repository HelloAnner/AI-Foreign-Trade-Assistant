...
- **口令管理**：设置页新增“访问口令”区域，仅输入一次新的管理员口令即可在线更新；前端使用 RSA 加密传输，后端落库前做 SHA-256 哈希。保存成功后会将 `login_password_version` 自增并嵌入 JWT，强制让全部已登录 token 立即失效，满足“修改密码后重新登录”的管控需求。
- **RSA 加密链路**：
...
- **数据挂载目录**：`scripts/start.sh` 恢复以宿主机 `~/.foreign_trade` 为默认数据路径（与历史版本一致），并挂载到容器 `/data`，自动注入 `FTA_DATA_DIR=/data`，因此升级后仍能直接复用原有数据库。若需要自定义，可导出 `FOREIGN_TRADE_DATA_DIR=/your/path`；脚本会继续 build+run，不再做健康检测。
