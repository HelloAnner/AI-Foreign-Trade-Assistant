...
mkdir -p "$DIST_DIR/windows_playwright"
GOOS=windows GOARCH=amd64 bash "$PLAYWRIGHT_SCRIPT" "$DIST_DIR/windows_playwright"
...
mkdir -p "$DIST_DIR/macos_playwright"
bash "$PLAYWRIGHT_SCRIPT" "$DIST_DIR/macos_playwright"

echo "✓ 打包完成，产物位于 $DIST_DIR"
echo ""
echo "目录结构："
echo "  AI_Trade_Assistant_Windows.exe"
echo "  windows_playwright/"
echo "  AI_Trade_Assistant_macOS_arm64"
echo "  macos_playwright/"
echo ""
echo "使用说明："
echo "  1. Windows: 双击 AI_Trade_Assistant_Windows.exe"
echo "  2. macOS: ./AI_Trade_Assistant_macOS_arm64"
echo "  程序会自动使用同目录下的 playwright 环境"
