...
// ErrNoAdminEmail indicates there is no configured admin inbox for outbound emails.
var ErrNoAdminEmail = errors.New("admin email not configured")
...
func (s *SchedulerServiceImpl) Schedule(ctx context.Context, req *domain.ScheduleRequest) (*domain.ScheduleResponse, error) {
	...
	if _, err := s.requireAdminEmail(ctx); err != nil {
		return nil, err
	}
...
func (s *SchedulerServiceImpl) RunNow(ctx context.Context, taskID int64) error {
	...
	recipients := []string{adminRecipient}
...
func (s *SchedulerServiceImpl) requireAdminEmail(ctx context.Context) (string, error) {
	...
	if admin == "" {
		return "", fmt.Errorf("%w: 请在设置中填写管理员邮箱作为默认收件人", ErrNoAdminEmail)
	}
	return admin, nil
}
